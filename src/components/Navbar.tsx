...
export const Navbar = ({ hundleNavbar, openBar }: navPropsType) => {
  ...
  return (
    <div className='flex flex-row justify-between items-center p-4 lg:py-6 lg:px-9 bg-white blue-200 border-b-2 border-gray-100 '>
      ...
      <div className='ml-7'><a href='index.html'><img src={logo} alt='logo' className='w-15 md:w-17' /></a></div>

      <div className='lg:hidden'><button onClick={hundleNavbar}>{!openBar ? <FiAlignJustify size='35' /> : <FiX size='35' />}</button></div>
      ...
      <nav className={`${openBar ? 'flex' : 'hidden lg:flex'}  shadow-hidden lg:h-auto lg:w-auto w-screen h-full lg:relative lg:top-0  fixed left-0 top-18  bg-[#ececfb] lg:bg-transparent`}>
        <ul className='pt-16 lg:pt-0 px-8  box-border relative flex flex-col lg:flex-row'>
          {
            navList2.map(item => (
              <li key={item.id} className={item.style}>{item.title}</li>
            ))
          }
        </ul>
      </nav>
